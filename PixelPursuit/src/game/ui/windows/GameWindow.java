...
/**
 * GameWindow - Top-level window for running a game session. - Builds maze,
 * runner, chasers, and in-game HUD. - Owns the GamePanel and end-of-run summary
 * dialog flow.
 */
public class GameWindow extends JFrame {
	...
	public GameWindow(WindowManager windowManager, Account account) {
		...
		this.lootDisplay = new LootDisplayPanel(0, // starting run gold
	...
	// isPaused - Returns true if the run is currently paused
	public boolean isPaused() {
		return paused;
	}

	// setPaused - Updates paused state and forwards it to the GamePanel
	public void setPaused(boolean paused) {
		this.paused = paused;

		// Tell the GamePanel so it can stop its timer / updates.
		if (gamePanel != null) {
			gamePanel.setPaused(paused);
		}
	}

	// Called when the player hits ESC
	private void requestPause() {
		...
		PauseWindow pause = new PauseWindow(this, windowManager);
		pause.setLocationRelativeTo(this);
		pause.setVisible(true); // code will block here until pause closes
	}

	// Called by PauseWindow when "Resume" is clicked
	public void resumeGameFromPause() {
		setPaused(false);
		SwingUtilities.invokeLater(gamePanel::requestFocusInWindow);
	}

	// Called by PauseWindow when "Return to Menu" is clicked
	public void returnToMenuFromPause() {
		// Just leave the run and go back to main menu
		this.dispose();
	...
	// Wire ESC trigger requestPause() while the game window is focused
	private void installPauseKeyBinding() {
	...
	private void showEndOfRunDialog(boolean escaped) {
		...
		SessionResult result = scoreSystem.compute(session, difficulty, escaped, accountMultIndex);
	...
	// getEquippedMultiplierValue - Returns the equipped account multiplier value
	// (2,3,5,10) or 1
